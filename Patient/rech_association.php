...
$requete2="SELECT distinct categorie FROM `associations`";
...
if (str.length==0)
  { 
  document.getElementById("txtHintasso").innerHTML="";
  return;
  }
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
    document.getElementById("txtHintasso").innerHTML=xmlhttp.responseText;
    }
  }
xmlhttp.open("GET","live_search(asso).php?q="+str,true);
xmlhttp.send();
}
</script>
</head>
...
header("location:rech_association.php?id=".$ligneR['id_patient']);
}
?>
...
<body>			       
	...
	<div class="rechercheassociation">
	   <h3>Rechercher une association</h3>
	</div></br></br></br>
	<div class="gh_recherche_association">
	<center>
	     <form method="post" action="recherche_association.php?id=<?php echo $ligne['id_patient'];?>" name="recherche_association">
		 <label>Rechercher par : </label> <label> <input type="radio" name="type_asso" id="type_asso" value="nom"/>Nom</label>
            <label> <input type="radio" name="type_asso" id="type_asso" value="categorie"  />Categorie</label><label> <input type="radio" name="type_asso" id="type_asso" value="tous" checked="checked" />Toutes les associations</label></br></br>
			<label>Nom : <input type="text" name="nom_asso" id="nom_asso" placeholder="Nom d'association" onkeyup="showHint(this.value)" value=""/></label><span id="txtHintasso"></span></br>
			<label>Categorie : </label> <select name="categ_asso" id="categ_asso" ><option value="0" selected="1">Categorie</option><?php while($ligne2=mysql_fetch_array($resultat2)){?> <option value="<?php echo $ligne2['categorie'];?>"><?php echo $ligne2['categorie'];?></option> <?php   }  ?></select></br></br>
			<input id="rech_asso" type="submit" value="Rechercher" /></br></br>
			</center>
		 </form>
	</div>
</body>
