...
<script type="text/javascript" src="./js/Chart.js"></script>
 ...
 var page=1;
 var per_page=3;
 var items= $('.gh_liste_medecin').length;
 var page_last=Math.ceil(items/per_page);
 
 function setPage(page){
 $('.gh_liste_medecin').slice(0, page * per_page).hide();
 $('.gh_liste_medecin').slice(page * per_page - per_page , page * per_page).show();
 $('.gh_liste_medecin').slice(page * per_page).hide();
...
$type=$_POST['type_medecin'];
$ordre=$_POST['ordre_medecin'];
if($type=="np" && $ordre=="o")
{
$nom=$_POST['nom_medecin'];
$prenom=$_POST['prenom_medecin'];
$requete2="SELECT * FROM `medecin` WHERE nom='$nom' and prenom='$prenom' order by note/nb_vote desc";
}

if($type=="s" && $ordre=="o")
{
$spe=$_POST['sp_med'];
$requete2="SELECT * FROM `medecin` WHERE specialite='$spe' order by note/nb_vote desc";
}
...
$prenom=$_POST['prenom_medecin'];
$spe=$_POST['sp_med'];
$requete2="SELECT * FROM `medecin` WHERE nom='$nom' and prenom='$prenom' and specialite='$spe' order by note/nb_vote desc";
}
...
$nom=$_POST['nom_medecin'];
$prenom=$_POST['prenom_medecin'];
$requete2="SELECT * FROM `medecin` WHERE nom='$nom' and prenom='$prenom' ";
}

if($type=="s" && $ordre=="n")
{
$spe=$_POST['sp_med'];
$requete2="SELECT * FROM `medecin` WHERE specialite='$spe'";
}
...
$prenom=$_POST['prenom_medecin'];
$spe=$_POST['sp_med'];
$requete2="SELECT * FROM `medecin` WHERE nom='$nom' and prenom='$prenom' and specialite='$spe'";
}
...
if($type=="tous" && $ordre=="n")
{
$requete2="SELECT * FROM `medecin`";
}

if($type=="tous" && $ordre=="o")
{
$requete2="SELECT * FROM `medecin` order by note/nb_vote desc";
}
...
<body>			       
	...
	<div class="gh_liste_medecin">
	     <center>
	       <span><?php echo $r=$r+1;?></span> <a href="medecin.php?id=<?php echo $ligne['id_patient'];?>&ida=<?php echo $ligne2['id_medecin'];?>" target="_blank" ><?php echo "Dr"." ".$ligne2['nom']." ".$ligne2['prenom'];?> ----------- <?php echo $ligne2['specialite'];?></a>
		   </center>
	</div>
		</br>
	<?php   }  ?>
