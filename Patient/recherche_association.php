...
<script type="text/javascript" src="./js/Chart.js"></script>
 ...
 var page=1;
 var per_page=3;
 var items= $('.gh_liste_association').length;
 var page_last=Math.ceil(items/per_page);
 
 function setPage(page){
 $('.gh_liste_association').slice(0, page * per_page).hide();
 $('.gh_liste_association').slice(page * per_page - per_page , page * per_page).show();
 $('.gh_liste_association').slice(page * per_page).hide();
...
$type=$_POST['type_asso'];
if($type=="nom")
{
$nom_asso=$_POST['nom_asso'];
$requete2="SELECT * FROM `associations` WHERE nom='$nom_asso' ";
}
if($type=="categorie")
{
$categ_asso=$_POST['categ_asso'];
$requete2="SELECT * FROM `associations` WHERE categorie='$categ_asso' ";
}
if($type=="tous")
{
$requete2="SELECT * FROM `associations` ";
}
...
<body>			       
	...
	<div class="gh_liste_association">
	     <center>
	       <span><?php echo $r=$r+1;?></span> <a href="association.php?id=<?php echo $ligne['id_patient'];?>&ida=<?php echo $ligne2['idAssociation'];?>" target="_blank"><?php echo $ligne2['nom'];?></a>
		   </center>
	</div>
	</br></br></br>
	<?php   }  ?>
