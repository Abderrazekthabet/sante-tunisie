...
class Partenaire
{
    ...
    public function authentification($usr, $pwd)
    {
        if($requete_authent = mysql_query("select * from utilisateurs where login='$usr' AND mdp='$pwd' AND type='admin_part';"))
		{
			...
			if ($rep=mysql_fetch_array($requete_authent))
			{
                ...
                $_SESSION['login']=$usr;
                $_SESSION['mdp']=$pwd;
                $_SESSION['type']="admin_part";
				header("refresh:0;url=../../Partenaires/$usr/index.php");
			}
			else
			{
    ...
    public function Ajouter()
    {
        ...
        $password = $this->nom."_123";
        $createCredentials = "INSERT into utilisateurs (id,login,mdp,type) values(NULL, '$this->nom', '$password','admin_part');";
        if(mysql_query($addPart) && mysql_query($createCredentials))
        {
            return TRUE;
        }
        else
    ...
    {
        mysql_query("delete from partenaires where nom='$nom'");
        mysql_query("delete from utilisateurs where login='$nom'");
        chdir("../../Partenaires/$nom");
        unlink("contact.php");
        unlink("index.php");
        chdir("../");
        rmdir($nom);
